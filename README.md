...
| 模块 | 功能 |
|------|------|
| **调度中心** | 多 Agent 统一调度引擎，基于 LangGraph StateGraph 实现「意图识别 → 路由 → 状态机 → 工具编排 → 结果汇总」全链路，支持对话式交互与产物注入 |
| **运营 Agent** | 商品 / Listing 查询问答与生成，支持多店铺多市场数据检索 |
| **Listing Agent** | Listing 文案生成、合规审查，基于产品资料自动生成优化文案 |
| **供应链 Agent** | 库存查询、补货点分析、仓库维度数据聚合，支持安全库存预警 |
| **广告 Agent** | 广告投放 ROI 分析、花费与销售额趋势、按商品/广告组排行 |
| **审批流** | 写操作必经人工审批，待审批 → 通过 / 拒绝 / 超时挂起，含变更前后值 diff 与依据 |
| **操作留痕** | 同操作 ID 贯穿「意图 → 审批 → 执行 → 留痕」，全量审计日志记录，支持按时间/Agent/操作类型筛选追溯 |
| **产物中心** | Agent 分析结果自动沉淀为产物，支持挑选注入到任意 Agent / 会话，跨会话共享分析洞察 |
| **数据看板** | 任务总数 / 完成率、工具调用次数 / 成功率、知识库检索量、Token 消耗等六大指标卡片 + 趋势图 |
| **知识库 RAG** | 文档 / 业务表一键索引 → 切分 → 向量化 → Qdrant 存储 → BM25 + 向量 RRF 融合 + BGE-rerank 重排，未安装模型自动降级轻量 Embedding |
| **工具注册表** | 字段驱动自动生成工具，业务表有哪些字段才生成哪些工具，降低误操作面 |
| **数据引擎** | 多店铺 / 多市场 / 多仓库 / 多币种数据模型（Amazon / TikTok / Walmart），每日动态补数保证时序分析始终有当日数据 |

### P1 功能增强

| 模块 | 功能 |
|------|------|
| **监控预警** | 可配置预警规则引擎（阈值/趋势/异常检测），预置规则种子，启动时自动加载调度器，预警记录自动沉淀为产物 |
| **联动引擎** | 事件总线架构，Agent 间通过 LinkageEvent 解耦通信，支持事件去重（24h 窗口）、循环检测（1 层）、深度限制（2 层）、全链路追踪（chain_id） |
| **老板视图** | 全局经营概览，聚合店铺 / 品类 / 市场维度核心指标，为管理层提供一站式数据快照 |
| **补货 Agent** | 基于库存 / 销量 / 交期自动计算补货建议，含安全库存预警、备货量建议、供应商交期分析 |
| **审批策略配置** | 白名单自动通过 / 审批人 / 超时时长可配置，审批记录全量可查 |

### P2 完整能力

| 模块 | 功能 |
|------|------|
| **采购比价 Agent** | 供应商管理 + 询价记录，按 SKU 聚合多供应商报价，综合 TCO（总拥有成本）评分，推荐最优供应商 |
| **预算分配 Agent** | 基于历史 ROI 的主动式预算分配策略，综合 ROAS 权重计算，风险评估（低/中/高），自动生成预算调整提案 |
| **跨场景组合策略** | 预置策略模板（选品机会→全链路协同、补货计划→成本与广告协同、竞品价格突变→多维度响应、差评/退货激增→多 Agent 响应），DAG 编排引擎，自动联动多 Agent 执行 |
| **工具自动扩展** | 新表 / 新字段自动发现，按当前数据库表结构重建工具集，零配置扩展 |
| **记忆 PII 治理** | 13 种 PII 类型检测（邮箱 / 电话 / 地址 / 证件号 / 银行卡等），支持拦截 / 脱敏两种模式，写入审核日志，容量上限与自动过期策略可配置，自动移除最旧条目 |
| **合规审计视图** | 合规审计概览看板（审批通过率、PII 拦截数、审计覆盖率），7 项 GDPR 合规检查清单，支持一键导出合规报告 CSV |
| **审批规则引擎** | 阈值自动规则，支持 8 种比较运算符（含百分比变化），规则命中自动放行或升级审批，字段级/表级规则灵活配置 |
...
│   │   ├── config.py           # 配置（.env 读取）
│   │   ├── api/                # REST 路由（审批/产物/看板/审计/知识库/工具/组合策略…）
│   │   ├── scheduler/          # LangGraph 调度中心 + 联动引擎 + 组合策略编排
│   │   ├── agents/             # 业务子 Agent（运营/Listing/供应链/广告）
│   │   ├── analysis/           # 分析引擎（聚合/图表/洞察/采购比价/预算分配/补货计算）
│   │   ├── approval/           # 审批流 + 阈值规则引擎
│   │   ├── audit/              # 操作留痕审计
│   │   ├── artifacts/          # 产物中心
│   │   ├── dashboard/          # 数据看板聚合
│   │   ├── compliance/         # 合规审计视图
│   │   ├── knowledge/ rag/     # 知识库 + 检索（BM25/向量/RRF/重排）
│   │   ├── memory/             # 记忆管理（PII 检测/容量/过期）
│   │   ├── monitor/            # 监控预警规则 + 调度器
│   │   ├── tools/              # 字段驱动工具注册表 + 技能插件
...
- **`honey_desk` 业务库**：商品 / 产品资料 / Listing / 销售订单 / 竞品快照 / 库存 / 广告数据 / 广告预算 / 供应商 / 询价；
- **`honey_system` 系统库**：店铺 / Agent / 工具 / 对话 / 任务 / 审批 / 产物 / 审计 / 知识库 / 记忆 / 设置 / 联动事件 / 预警记录。
...
- 写操作进入**审批流**：待审批 → 通过 / 拒绝 / 超时挂起，审批详情含目标表 / 字段 / **变更前后值 diff** / 依据，支持阈值规则引擎自动放行（新增量 < 5% 自动通过等）；
- 审计与业务数据同库存储，但通过「应用层只读约束 + 追加式写入」保证**只读防篡改**（REST 层仅暴露查询，无更新 / 删除接口）；
- 同一操作 ID 贯穿「意图 → 审批 → 执行 → 留痕」全链路；
- 记忆 PII 治理：13 种 PII 类型自动检测拦截，默认不入长期记忆，支持脱敏模式（GDPR 合规）；
...
- 功能建议 / 体验优化 → [Issues](https://github.com/Yvonnezy0923/HoneyDesk) 新建讨论；
